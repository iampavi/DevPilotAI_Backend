...
public class AzureOpenAIChatProvider : IChatProvider
{
    ...
    public AzureOpenAIChatProvider(HttpClient httpClient, IConfiguration configuration, ILogger<AzureOpenAIChatProvider> logger)
    {
        _httpClient = httpClient;
        _logger = logger;

        _apiKey = configuration["EmbeddingSettings:ApiKey"] ?? string.Empty;
        _endpoint = configuration["EmbeddingSettings:BaseUrl"] ?? string.Empty;
        _deploymentName = configuration["EmbeddingSettings:Model"] ?? string.Empty;
        _apiVersion = "2023-05-15";
